...
const YUUMI_CONFIG_DEFAULTS = {
    ...
    // Tables always visible regardless of active config selection.
    pinnedTables: ["tblKVUhPzuLJQwPB3"]
};

// Runtime config — populated asynchronously.
...
/**
 * Shared bootstrap used by ALL content scripts.
 */
function yuumiReady(callback, _retries = 40) {
    if (YUUMI_CONFIG !== null) {
        callback();
    } else if (typeof loadYuumiConfig === 'function') {
        loadYuumiConfig(() => callback());
    } else if (_retries > 0) {
        setTimeout(() => yuumiReady(callback, _retries - 1), 75);
    } else {
        console.warn('[Yuumi] config.js not ready after timeout.');
    }
}
